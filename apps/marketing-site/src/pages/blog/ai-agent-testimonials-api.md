...
`import httpx def submit_testimonial(author: str, text: str, rating: int = 5): response = httpx.post( "https://app.socialproof.dev/api/agent/register", headers={ "Authorization": f"Bearer {SOCIALPROOF_API_KEY}", "Content-Type": "application/json" }, json={ "widget_id": SOCIALPROOF_WIDGET_ID, "author": author, "text": text, "rating": rating, "source": "ai-agent" } ) response.raise_for_status() return response.json()` ## Wiring it into a GPT-4 function call Same pattern for OpenAI's function calling:
